...
export function NotificationBell() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/notifications", { cache: "no-store" })
      ...
      .then((payload) => {
        if (!cancelled && Array.isArray(payload?.notifications)) setRows(payload.notifications);
      })
      .catch(() => {
  ...
  return (
    <div className="relative">
      <button className="focus-button secondary" type="button" onClick={() => setOpen((current) => !current)} aria-label="Notificari">
        <Bell size={18} />
        Notificari
      ...
      {open ? (
        <div className="absolute right-0 top-full z-30 mt-2 w-[min(92vw,420px)] rounded-lg border border-focus-line bg-focus-navy p-3 shadow-2xl">
          <div className="mb-2 flex items-center justify-between gap-3 border-b border-focus-line pb-2">
            <strong className="text-sm uppercase text-focus-yellow">Scadente clienti</strong>
            ...
            {activeRows.length ? activeRows.map((row) => (
              <article className="grid gap-2 border-b border-focus-line py-3 text-sm last:border-b-0" key={row.id}>
                ...
                <div className="flex flex-wrap gap-2">
                  <button className="focus-button secondary" type="button" disabled={busy === `${row.id}-called`} onClick={() => action(row.id, "called")}><PhoneCall size={14} /> Am sunat</button>
