...
const workspaceService = read("src/lib/receivables-workspace-service.ts");
...
assert.match(workspace, /<select className="focus-input" name="currency"/);
assert.doesNotMatch(workspace, /window\.(prompt|confirm)/, "financial actions must use explicit application dialogs");
