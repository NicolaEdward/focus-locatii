...
assert(cooService.includes("decideAvailability({ ...location, periodStart: today, periodEnd: today, now })"), "COO inventory must use the canonical availability decision");
assert(cooService.includes('periodStart: { lte: today }, periodEnd: { gte: today }'), "COO current occupancy must not load future HOLD periods");
