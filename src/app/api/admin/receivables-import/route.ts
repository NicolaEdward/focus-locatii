...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (file.size > 20 * 1024 * 1024) return NextResponse.json({ error: "Fișierul depășește limita de 20 MB." }, { status: 400 });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Raportul nu a putut fi analizat." }, { status: 400 });
  }
}
